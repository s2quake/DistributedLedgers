...
sealed partial class Algorithm_2_10Command : CommandAsyncBase
{
    ...
    protected override async Task OnExecuteAsync(CancellationToken cancellationToken, IProgress<ProgressInfo> progress)
    {
        ...
        await using var client1 = await Client.CreateAsync(serverPorts);
        await using var client2 = await Client.CreateAsync(serverPorts);
        await using var client3 = await Client.CreateAsync(serverPorts);
